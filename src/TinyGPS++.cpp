...
double TinyGPSLocation::peekLat()
{
   double ret = rawLatData.deg + rawLatData.billionths / 1000000000.0;
   return rawLatData.negative ? -ret : ret;
}
...
double TinyGPSLocation::peekLng()
{
   double ret = rawLngData.deg + rawLngData.billionths / 1000000000.0;
   return rawLngData.negative ? -ret : ret;
}
...
uint8_t TinyGPSTime::peekMinute()
{
   return (time / 10000) % 100;
}
...
uint8_t TinyGPSTime::peekSecond()
{
   updated = false;
   return (time / 100) % 100;
}
...
uint8_t TinyGPSTime::peekCentisecond()
{
   updated = false;
   return time % 100;
}
